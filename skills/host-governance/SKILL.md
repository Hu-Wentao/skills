...
Use the authoritative host repository and resolve every operation before execution. Repository context is evidence, never live state.
...
```
Read returned policy references and use the runner only. Add `--authorized` only for a mutating operation covered by the current request. Read `references/project_config.md` before profile changes.

Route product details to the smallest reference: `control.md`, `server-bootstrap.md`, `docker-*.md`, `github-actions-runner.md`, `jenkins.md`, `postgresql.md`, `tailscale.md`, `caddy.md`, or `cloudflare*.md`.
...
- One-time repairs compose existing contracts. Productize only on an explicit request to create a reusable capability.
