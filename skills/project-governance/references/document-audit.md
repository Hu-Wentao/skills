...
Require every governed requirements, baseline, plan, defect, archive, coverage,
verification, or traceability Markdown document to have a valid persistent mdq
contract. Run queryable-markdown collection validation across the governed
paths; a textual `mdq:` mention is not proof of a valid contract. Treat a
missing contract, invalid profile, unsafe index path, ambiguous identity, or
undeclared lifecycle field required by the project as structural drift.
